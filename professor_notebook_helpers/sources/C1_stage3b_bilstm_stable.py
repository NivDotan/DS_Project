...
def run_stage3b_stable(cfg: Stage3BStableCfg):
    ...
    for t_reject in cfg.t_reject_choices:
        ...
        trej_tag = f"{int(round(float(t_reject) * 100)):03d}"
        phaseB_ckpt_path = os.path.join(out_dir, f"stage3b_phaseB_last_trej_{trej_tag}.pt")
        torch.save(
            {
                ...
                "extra": {
                    "cfg": asdict(cfg),
                    "t_reject": float(t_reject),
                    "phase": "phaseB_complete",
                },
            },
            phaseB_ckpt_path,
        )
        print(f"[PhaseB] saved pre-conf checkpoint: {phaseB_ckpt_path} | exists={os.path.exists(phaseB_ckpt_path)}")
...
# ==============================================================================
# BiLSTM Stage3B override + m_fix_bilstm run config
# Source: Handling Class Imbalance.ipynb cell 20
# ==============================================================================


# Fix loader key mapping for Stage3 BiLSTM checkpoints
def _extract_state_any(ckpt):
    # unwrap
    sd = ckpt
    ...
    # map rnn.*  -> net.2.* (for compatibility with your cell-18 code)
    if isinstance(sd, dict):
        has_rnn = any(k.startswith("rnn.") for k in sd.keys())
        has_net2 = any(k.startswith("net.2.") for k in sd.keys())
        if has_rnn and not has_net2:
            remap = {}
            for k, v in sd.items():
                if k.startswith("rnn."):
                    remap["net.2." + k[len("rnn."):]] = v
                else:
                    remap[k] = v
            sd = remap
...
class Stage3ABCMXTCN(nn.Module):
    """
    BiLSTM backbone with compatibility for existing cell-18 code:
      - exposes .net
      - supports .net[2]
      - supports .net.parameters()
    """
    def __init__(self, num_features: int, hidden: int = 64, layers: int = 1, bidirectional: bool = True):
        super().__init__()
        ...
        # Compatibility shim: existing code uses net[2]
        self.net = nn.ModuleList([nn.Identity(), nn.Identity(), rnn])

        out_dim = hidden * (2 if self.bidirectional else 1)
        self.head_reject = nn.Linear(out_dim, 2)  # quiet/flare
        self.head_bcmx = nn.Linear(out_dim, 4)    # B,C,M,X

    def forward(self, x):
        # x: [N,T,F]
        h_seq, _ = self.net[2](x)   # BiLSTM
        h = h_seq.mean(dim=1)       # temporal pooling
        return self.head_reject(h), self.head_bcmx(h)
...
cfg_m_fix = Stage3BStableCfg(
    ...
    s1_ckpt_path=r"runs_pytorch_stage2_fix\W24_H24_two_stage_bal_other_stage1\stage1_last.pt",
    # IMPORTANT: this must be a BiLSTM Stage3A checkpoint (NOT TCN)
    s3a_ckpt_path=r"runs_pytorch_stage2_fix\W24_H24_stage3a_reject_only_bilstm\W24_H24_stage3a_reject_only\stage3a_reject_last.pt",
)
...
# Use the already-trained PhaseA checkpoint as starting point
cfg_m_fix.s3a_ckpt_path = r"runs_pytorch_stage2_fix\m_fix_bilstm_v1\W24_H24_stage3b_stable\stage3b_phaseA_last.pt"

# Skip PhaseA training loop
cfg_m_fix.phaseA_epochs = 0
cfg_m_fix.save_phaseA_each_epoch = False
cfg_m_fix.save_phaseA_last = False


cfg_m_fix.batch_files = 1
cfg_m_fix.per_file = 64
cfg_m_fix.fixed_batch_size = 128
cfg_m_fix.phaseB_quota_total = 128
cfg_m_fix.s3a_val_npz = r"runs_pytorch_stage2_fix\W24_H24_stage3a_reject_only_bilstm\W24_H24_stage3a_reject_only\val_arrays_stage3a.npz"
